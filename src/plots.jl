...
function plot_profile_var(sols...; var_name=:r)
    ...
    for (sol, color) in zip(sols, colors)
        ...
        elseif var_name == :qv
            ...
            plot!(p_var, qv_env .* 1.0e3, z_prof, label=nothing, color=color, linestyle=env_linestyle)
            plot!(p_var, qv_sat_cld .* 1.0e3, z_prof, label=nothing, color=color, linestyle=:dashdot)
        elseif var_name == :rh
            plot!(p_var, rh_c, g(sol, :z), label="", xlabel="relative humidity [1]", color=color)
        ...
        elseif var_name == :Δrho
            ...
            plot!(p_var, Δrho, z_prof, xlabel="Δρ [g/m^3]", color=color, label=nothing)
        elseif var_name == :ql
